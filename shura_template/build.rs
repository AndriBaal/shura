...
fn copy_directory(src: &Path, dest: &Path) -> io::Result<()> {
    const EXCLUDE_EXTENSIONS: &[&str] = &[
        "blend", "blend1", "aseprite", "ase", "aup3-shm", "aup3", "aup3-wal",
    ];
    ...
    match target_os.as_ref().map(|x| &**x) {
        ...
        Ok(_) => {
            ...
            if resources.exists() {
                ...
                if dest.exists() {
                    copy_directory(&resources, &dest.join("resources")).unwrap();
                    let examples = dest.join("examples");
                    if examples.exists() {
                        copy_directory(&resources, &examples.join("resources")).unwrap();
                    }
                }
            }
        }
